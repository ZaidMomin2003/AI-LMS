    ...
    <div className="group w-full max-w-xs h-48 perspective-1000">
      <div className="relative w-full h-full cursor-pointer transition-transform duration-700 transform-style-preserve-3d group-hover:rotate-y-180">
        ...
        <div className="absolute w-full h-full backface-hidden">
          <Card className="h-full flex flex-col justify-between p-6 bg-card/50 shadow-xl border-2 border-border/10 backdrop-blur-sm">
...
const RadioOption = ({ text, selectedOption, correctAnswer, onSelect }: { text: string; selectedOption: string | null; correctAnswer: string; onSelect: (option: string) => void; }) => {
    ...
    let stateVariant = "bg-secondary hover:bg-accent";
    if (selectedOption) {
        if (isSelected && isCorrect) {
            stateVariant = "border-green-500 bg-green-500/10";
        } else if (isSelected && !isCorrect) {
            stateVariant = "border-red-500 bg-red-500/10";
        } else if (isCorrect) {
            stateVariant = "border-green-500/50 bg-green-500/5 text-muted-foreground";
        } else {
             stateVariant = "border-border bg-card text-muted-foreground opacity-60";
        }
    }
...
function InteractiveQuiz() {
  ...
  return (
    <Card className="w-full max-w-md bg-card/50 p-4 shadow-lg border-2 border-border/10 transition-transform duration-300 hover:scale-105">
        <CardContent className="p-2 space-y-3">
            ...
            {options.map((option) => (
             ...
             {selectedOption && selectedOption !== correctAnswer && (
                 <p className="text-sm text-red-500 pt-2 font-medium">Not quite. The correct answer is highlighted in green.</p>
            )}
            {selectedOption === correctAnswer && (
                 <p className="text-sm text-green-500 pt-2 font-medium">Correct! Well done.</p>
            )}
        </CardContent>
    </Card>
  )
    ...
    <Card className="w-full max-w-md bg-card/50 p-4 shadow-lg border-2 border-border/10 transition-transform duration-300 hover:scale-105">
      <CardContent className="p-2 space-y-3">
        <div className="flex items-start gap-3">
            ...
            <div className="rounded-2xl p-3 bg-secondary backdrop-blur-sm rounded-bl-none max-w-[80%]">
        ...
        <Card className="w-full max-w-md bg-card/50 p-4 shadow-lg border-2 border-border/10 transition-transform duration-300 hover:scale-105">
            ...
            <CardContent className="p-2 space-y-2">
                ...
                <Card className="bg-secondary p-3">
                 ...
                 <Card className="bg-secondary p-3 opacity-50">
...
function StudyPlanDemo() {
    const DemoTask = ({ title }: { title: string }) => (
        <div className="p-2 mb-2 bg-background rounded-md shadow-sm border">
...
function ExamTimerDemo() {
    const TimeCard = ({ value, unit }: { value: string, unit: string }) => (
        <div className="flex flex-col items-center bg-secondary p-2 rounded-lg flex-1">
    ...
    <Card className="w-full max-w-md bg-card/50 p-4 shadow-lg border-2 border-border/10 transition-transform duration-300 hover:scale-105">
      <CardContent className="p-2 flex items-center justify-center gap-4">
        <Image
          ...
          width={150}
          height={112}
          className="rounded-md border-2 border-dashed border-border"
        />
